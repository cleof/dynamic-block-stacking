...
def blockOrientations(dimension):
	...
	h = dimension[2]
	orientations = []
	# add all permutations where depth <= width
...
# output: the maximum stack height, the number of blocks used, and list of blocks
def blockStacking(dimensions):
	...
	# find maximum from DP table and index of maximum
	maximum = 0
	max_index = -1
...
# read input file and return solution
def read_file(input):
	with open(input,'r') as i:
		lines = i.readlines()
	dims = []
	# make list of dimension tuples
	for line in lines:
...
# generate list of strings to print to output file
...
# read file, format solution and print to output file
